...
Two dataset of store information, reservation data and visit data from each air and hpg data systems were merged together to provide a whole data sets for each system. The missing values were deleted. Then both data system were combined using relation information of restaurants to generate our main dataset. However, there was not the related information for some of the entries.
This dataset contains store_id, date and time, genre, location and holiday which are categorical data. There are 517 unique dates, 4947 unique stores, 44 different genres, and 186 different area name. There were no missing data. However, this project confronting a few problems. One of them is the is no ranking or rating information for a restaurant. Another issue is the high number of categorical variables. Also, there is not an equal number of entries, or observations, for every restaurant. There is only one entry for some restaurants whereas there are lots of entries for others. This could happen if some restaurants are taking reservation more by phone rather than via websites, or they are more likely to be the fast-food type of stores.

The data include datetime type for reservation and visiting. The year, month, day, day of the week, time and time difference between reservation and visit calculated. Then regarding a large number of categorical features, the categorical data were converted to the numerical data by grouping visitors over hour, day, day of the week, month, year, store, genre and the location. This method also generates extra information. Please check the [code] 
(https://github.com/leilanorouzi/SpringBoard/blob/master/Capstone_project1/Codes/Restaurant_forcats_data_processing.ipynb)
This information can help to find the best feature for our model and finding a solution to identify how favorable their restaurants are for the public, please see "Extracting new data" section.


There are an extensive data visulazation in output directory, https://github.com/leilanorouzi/SpringBoard/tree/master/Capstone_project1/Files/Outputs/Graphes.  The [visualization code](https://github.com/leilanorouzi/SpringBoard/blob/master/Capstone_project1/Codes/Restaurant_forcats_features_visual.ipynb) can be found inthe code directory. 
...
### Extracting new data
The project contains lots of categorical data.  In this case, I have the genre of the restaurant, day of the week, the name of the restaurant location, holiday, date, and etc. One approach is encoding them. To do that, they can be converted into a big set of ones and zeros (hot-encoding). This method may be problematic since the number of categorical features is high. The other solution can be the extraction of some information from those categorical data.  
...
To do so, I used histograms information on the distribution of visitors with respect to different variables. Then I have introduced new factors as the ratio of each height of the bar to the maximum height of bars as (hight_bar/Max(hight_bar).
...
### Input of the model
TThe categorical features, genre name, area name, day, month, year and day of the week, were encoded. 
...
Then these data were split into train and test datasets to use in the model.
